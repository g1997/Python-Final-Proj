...
def login():
    cur=dbconn.cursor()
    sql="select * from owner_login where id='{}' and pwd='{}'".format(idtxt.get(),pwdtxt.get())
    cur.execute(sql)
    if(cur.fetchall()):
        showinfo(title="Success",message="Record Found")
        win.destroy()
        from subprocess import Popen
        Popen(["python","owner.py"])
    else:
...
ids=Label(text="Enter owner id:",bg="azure",fg="black",font=("Times New Roman",15))
